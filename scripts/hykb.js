  ...
  return new Promise(async (resolve) => {
    try {
      ...
      let res = await axios.post(
        ...
        `ac=${b}&${d}=${c}&r=0.1362954162068364&scookie=${scookie}`,
        {
          headers: {
            "User-Agent":
...
async function task() {
  ...
  if (logindata.key == "ok") {
          ...
          if (ress.option1 && ress.expand) {
            ...
            if (resss.data.result) {
              let strr = JSON.stringify(resss.data.result.data.downinfo.appinfo)
                .replace(/&nbsp;/g, "")
                .replace(/ /g, ""); //查答案
              reg = /错误|不属于|不是|不存在|没有|不能|不可以|不需要|无法/;
            ...
            await get(
              "grow",
              `DailyDatiAnswer&option=${ress["option" + kw]}`,
              4
            );
          } else {
    ...
    let csdata = await get("grow", "Dailylogin", "174");
