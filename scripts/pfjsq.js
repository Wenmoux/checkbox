...
class PFJS {
  ...
  async exchange() {
    const url =
      this.host + "client/api/v1/virtual_currency/exchange_by_species";
    try {
      this.body = '{"rule_id":4}';
  ...
  async sign() {
    const url =
      this.host + "client/api/v1/virtual_currency/sign_in_for_species";
    try {
      const res = await axios.post(url, this.body, { headers: this.headers });
  ...
  async getAd() {
    ...
    try {
      ...
      console.log(
        `广告${res.data.data.sign_count}/${res.data.data.limit_count}`
      );
  ...
  async lookAd() {
    const url =
      this.host + "client/api/v1/virtual_currency/look_ad_for_species";
    try {
      const res = await axios.post(url, this.body, { headers: this.headers });
