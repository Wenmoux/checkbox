  ...
  return new Promise(async (resolve) => {
    try {
      ...
      data =
        "growthStr=" +
        encodeURIComponent(
          '{"growthRecordDtos":[{"userId":' +
            info.uid +
            ',"clientVersion":"","playDuration":"' +
            Math.floor(Math.random() * -30 + 10800) +
            '","clientType":"web","objId":"' +
            Math.floor(Math.random() * 99 + 153300) +
            '","type":"season","playTime":"' +
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      if (code == "0000") {
        ...
        await answerq(info.answerid),
          await openbox("铜宝箱", "openboxt", `boxId=3&token=${rrtoken}`);
        await openbox("银宝箱", "openboxy", `boxId=2&token=${rrtoken}`);
        await openbox("金宝箱", "openboxgold", `boxId=1&token=${rrtoken}`);
      } else {
