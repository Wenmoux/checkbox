...
if [ ! -f "/ql/data/config/config.yml" ]; then
 ...
 cp /ql/data/repo/Wenmoux_checkbox_master/config.yml.temple /ql/data/config/config.yml
else
 echo "已存在配置文件"
 echo "生成空白模板 /ql/data/config/config.yml.sample"
 cp /ql/data/repo/Wenmoux_checkbox_master/config.yml.temple /ql/data/config/config.yml.sample
fi
