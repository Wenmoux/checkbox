...
if [ ! -f "/ql/config/config.yml" ]; then
 ...
 cp /ql/repo/Wenmoux_checkbox_master/config.yml.temple /ql/config/config.yml
else
 echo "已存在配置文件"
 echo "生成空白模板 /ql/config/config.yml.sample"
 cp /ql/repo/Wenmoux_checkbox_master/config.yml.temple /ql/config/config.yml.sample
fi
